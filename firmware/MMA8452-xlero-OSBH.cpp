...
byte MMA8452Q::init(MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
	...
	writeRegister(CTRL_REG2, 0x11); // Set MODS and SMODS to low noise low power mode (8uA)
	// Multiply parameter by 0.0625g to calculate threshold.
	setupTap(0x10, 0x10, 0x10); // Enable x, y, z set to 0.5g
	setupINT();	// setup interrupt INT1 for PULSE
...
void MMA8452Q::setupTap(byte xThs, byte yThs, byte zThs)
{
	...
	writeRegister(PULSE_CFG, temp | 0xFF);
...
//	lower 7 bits of the PULSE_SRC register.  Clears interrupt flag in INT_SOURCE.
...
// SET UP INTERRUPT on INT1
//	This function sets up portrait and landscape detection.
void MMA8452Q::setupINT()
{
	// Must be in standby mode to make changes!!!
	// For more info check out this app note:
	//	http://cache.freescale.com/files/sensors/doc/app_note/AN4068.pdf
	// 1. Enable SRC_PULSE
	writeRegister(CTRL_REG3, 0x02); // Set IPOL (enable), active HIGH for INT pin
	writeRegister(CTRL_REG4, 0x08); // Set SRC_PULSE (enable), cleared when PULSE_SRC is read
	writeRegister(CTRL_REG5, 0x08); // Set INT_CFG_PULSE (enable), interrupt source is directed to INT1 pin
}
